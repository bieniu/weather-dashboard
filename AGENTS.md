...
- DB migrations: `init_db()` in `backend/app/database.py` applies schema migrations from the `_MIGRATIONS` list. When adding a new column to `WeatherReading` model, add it to `_MIGRATIONS` and update `test_init_db_adds_missing_columns` to cover it. One-off data fixes (e.g. renaming a sensor key) go in `_DATA_MIGRATIONS` as idempotent SQL statements.
